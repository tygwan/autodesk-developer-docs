...
url_path: tutorials/prep-roominfo4viewer/task2-upload_source_file_to_oss
...
For this task, you upload the Revit file _rme_advanced_sample_project.rvt_ to OSS, which is available in the _walkthrough_data_ folder of the GitHub repository containing the Postman Collection for this walkthrough. This Revit file is an MEP file that contains space and zone information.
...
The first thing to do is to create a Bucket to hold the Revit model. Once a Bucket is created, you can store files in it as Objects.
...
- The bucket key must be unique throughout the OSS service. If the bucket key is in use (even by another user) the system returns a `409 Conflict` error. In such a case, retry with another bucket key.
...
- Replace <YOUR_OBJECT_KEY> with _rme_advanced_sample_project.rvt_, which is the name of the file we recommend that you upload.
...
- Download the file _rme_advanced_sample_project.rvt_ from [https://github.com/autodesk-platform-services/aps-tutorial-postman/tree/master/ModelDerivative_07/walkthrough_data](https://github.com/autodesk-platform-services/aps-tutorial-postman/tree/master/ModelDerivative_07/walkthrough_data).
...
---
원본 문서: https://aps.autodesk.com/en/docs/model-derivative/v2/tutorials/prep-roominfo4viewer/task2-upload_source_file_to_oss
