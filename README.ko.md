...
아래 블록 전체를 Claude Code 또는 Codex에 그대로 복사해 입력하세요. 에이전트가 환경을 확인하고, 적용 가능한 설정 선택지를 제시하고, 이 README의 체크아웃 설치 절을 사용해 결과를 검증한 뒤 실제 사용 안내까지 마치도록 요청하는 bootstrap 프롬프트입니다.

```text
Autodesk Developer Documentation 플러그인을 설정하고 사용법까지 안내해줘.

소스:
- 저장소: https://github.com/tygwan/autodesk-developer-docs
- 배포 ref: main-test
- 예상 플러그인: autodesk-developer-docs
- 자동 활성화될 워크플로: autodesk-api-implementer

다음 순서로 진행해줘.
1. 현재 세션이 Claude Code인지 Codex인지 식별해. 먼저 읽기 전용 검사로 예상 플러그인이 이미 설치·활성화되어 있는지, 유효한 소스 체크아웃이 있는지 확인해. 유효한 저장소 루트에는 CATALOG.json, .claude-plugin/plugin.json, .codex-plugin/plugin.json이 있어야 해.
2. 유효한 체크아웃 또는 배포 ref에서 README를 읽고 "AI 코딩 에이전트 시작점"과 "체크아웃한 저장소에서 설치" 절을 확인해. "체크아웃한 저장소에서 설치"의 명령을 설치 명령의 기준으로 사용하고 임의의 설치 명령을 만들지 마. README에 접근할 수 없다면 추측하지 말고 체크아웃 경로를 물어봐.
3. 로컬 또는 사용자 수준 상태를 변경하기 전에 적용 가능한 설정 선택지만 제시해: 검증된 기존 설치 사용, 기존 체크아웃에서 설치, 또는 배포 ref의 소스 체크아웃을 받은 뒤 그 체크아웃에서 설치. 적용 가능한 각 선택지가 무엇을 변경하는지 설명하고 내가 선택하게 해.
4. 내가 선택한 뒤 승인된 설정만 수행해. 현재 환경에서 필요한 명령을 실행할 수 없다면 정확한 명령을 알려주고 그 결과를 기다려. 플러그인 설정 중에는 Autodesk 자격 증명을 요구하지 마.
5. autodesk-developer-docs 플러그인과 autodesk-api-implementer 워크플로를 발견할 수 있는지 검증해. 플러그인 메타데이터를 다시 불러오기 위해 새 대화를 시작해야 한다면 명확히 알리고, 새 대화에 붙여 넣을 한 줄짜리 후속 프롬프트를 제공해.
6. 검증 후에도 onboarding을 마무리해: 자동 활성화, 지원하는 요청 유형, 보관된 아카이브 범위, 스냅샷과 현재 상태의 차이를 설명해. 내 프로젝트나 실행 환경을 알 수 있다면 거기에 맞춘 자연어 요청 예시 세 개를 제공하고, 첫 Autodesk 개발 작업을 물어봐.

설치 명령을 나열한 뒤 멈추지 말고 검증과 사용 안내까지 계속해. 내가 워크플로 이름을 직접 호출하도록 요구하지 마.
```
