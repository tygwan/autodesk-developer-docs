---
operation_id: integrate-updateelementwithoutrevision-POST
method: POST
path: /forma/integrate/v1alpha/elements/{elementId}
group: "integrate"
...
# Creates a new element with a new revision under the specified element ID

> ⚠️ **DEPRECATED** — 이 엔드포인트는 더 이상 권장되지 않습니다.

```http
POST https://developer.api.autodesk.com/forma/integrate/v1alpha/elements/{elementId}
```
...
Creates a new element with a new revision under the specified
element ID. Useful to keep track of changes in geometry in the same
logically evolving entity.
...
| `nextRevision` | string |  | Optionally specify the element revision. Must be an epoch timestamp in ms. |
...
| 코드 | 의미 | 설명 |
| --- | --- | --- |
| `200` | OK | Reference to the root of the newly created element tree |
...
```
curl -v 'https://developer.api.autodesk.com/forma/integrate/v1alpha/elements/:elementId?authcontext=pro_123' \
  -X 'POST' \
...
---
원본 문서: https://aps.autodesk.com/en/docs/forma/v1/reference/http-reference/integrate-updateelementwithoutrevision-POST
