---
operation_id: reviews-createworkflow-POST
method: POST
...
- `additionalApprovalStatusOptions` — `array: object`  
...
- `attachedAttributes` — `array: object`  
    (Update Attributes in the UI) An optional list of custom attribute IDs to attach to the workflow. These attributes will be applied to the approved files in the target folder (or optionally also in the source folder) after the review is completed.
  - `id` — `int` **(필수)**  
      The ID of the custom attribute to attach to the workflow.
  - `required` — `boolean`  
...
| `409` | Conflict | A workflow with the same name already exists in the project. |
...
---
원본 문서: https://aps.autodesk.com/en/docs/acc/v1/reference/http/reviews-createworkflow-POST
