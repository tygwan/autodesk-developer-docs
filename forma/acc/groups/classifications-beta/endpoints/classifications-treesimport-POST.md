---
operation_id: classifications-treesimport-POST
method: POST
path: /construction/classifications/v1/projects/{projectId}/trees:import
...
```http
POST https://developer.api.autodesk.com/construction/classifications/v1/projects/{projectId}/trees:import
```
...
Creates a new classification tree by importing a structured hierarchy of nodes, and publishes it as the latest version (tip version).
...
- `importData` — `array: object` **(필수)**  
    A list of root nodes for the tree. Each node can include nested children to represent the hierarchy. The full payload supports up to 50000 total nodes across all levels, with a maximum nesting depth of 10.
  - `id` — `string: UUID`  
...
| 코드 | 의미 | 설명 |
| --- | --- | --- |
| `201` | Created | Successfully created the classification tree |
...
```
curl -v 'https://developer.api.autodesk.com/construction/classifications/v1/projects/0195f158-6f95-7fb0-b008-4cde506166dd/trees:import' \
  -X 'POST' \
...
---
원본 문서: https://aps.autodesk.com/en/docs/acc/v1/reference/http/classifications-treesimport-POST
