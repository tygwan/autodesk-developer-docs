---
operation_id: sheets-storage-POST
method: POST
path: /construction/sheets/v1/projects/{projectId}/storage
group: "Sheets"
...
```http
POST https://developer.api.autodesk.com/construction/sheets/v1/projects/{projectId}/storage
```
...
Creates a storage location in the Object Storage Service (OSS) for you to upload the file to. This endpoint is typically used during the process of uploading files to the Forma Sheets tool.
...
- `fileName` — `string`  
    The name of the file. Currently Sheets tool only supports PDF files. Max length: 255
...
- `urn` — `string`  
    The URN of the storage. Note that in Sheets each project has its own OSS bucket, it is recommended to extract bucket key from storage URN every time before you call OSS APIs.
...
---
원본 문서: https://aps.autodesk.com/en/docs/acc/v1/reference/http/sheets-storage-POST
