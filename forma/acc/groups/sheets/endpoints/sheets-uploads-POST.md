---
operation_id: sheets-uploads-POST
method: POST
path: /construction/sheets/v1/projects/{projectId}/uploads
group: "Sheets"
...
# Creates an Forma upload object

```http
POST https://developer.api.autodesk.com/construction/sheets/v1/projects/{projectId}/uploads
```
...
Creates an Forma upload object. This endpoint splits the files into separate sheets.
It is typically used during the process of uploading files to Forma Sheets tool.
...
- `versionSetId` — `string` **(필수)**  
    The ID of the version set you want to upload the sheets to. To create a new version set, call POST version-sets. To find the ID of an existing version set, call GET version-sets.
- `files` — `array: object` **(필수)**  
    The list of files to be processed by the upload.
  - `storageType` — `enum:string` **(필수)**  
      The storage type of the file. Will always be OSS.
  - `storageUrn` — `string` **(필수)**  
      The storage URN of the file.
  - `name` — `string` **(필수)**  
      The name of the file. Currently the Sheets tool only supports PDF files. To create a storage URN, call POST storage. Max length: 255
...
- `versionSetId` — `string: UUID`  
    The ID of the version set where the upload creates sheets to.
- `status` — `enum:string`  
    The status of the upload. Possible values: - PENDING: the uploaded files are waiting for to be processed. - PROCESSING: the uploaded files are being processed. - IN_REVIEW: the file upload process is complete. The sheets are ready for review. You can now call GET review-sheets, PATCH review-sheets, or POST review-sheets:publish. - FAILED: the file upload process failed. One of the final status of an upload. - UPDATING_VERSION_SET: the target version set is being updated. - PUBLISHING: the review sheets are being published. - PUBLISHED: the review sheets have been published.
...
---
원본 문서: https://aps.autodesk.com/en/docs/acc/v1/reference/http/sheets-uploads-POST
