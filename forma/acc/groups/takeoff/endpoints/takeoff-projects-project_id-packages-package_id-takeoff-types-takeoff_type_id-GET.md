---
operation_id: takeoff-projects-project_id-packages-package_id-takeoff-types-takeoff_type_id-GET
method: GET
path: /construction/takeoff/v1/projects/{projectId}/packages/{packageId}/takeoff-types/{takeoffTypeId}
group: "Takeoff"
...
```http
GET https://developer.api.autodesk.com/construction/takeoff/v1/projects/{projectId}/packages/{packageId}/takeoff-types/{takeoffTypeId}
```
...
| `takeoffTypeId` | string: UUID |  | The takeoff type ID. To find the ID, call GET takeoff-types. |
...
- `id` — `string: UUID`  
    The takeoff type ID.
- `name` — `string`  
    The takeoff type name. Max length: 64
- `description` — `string`  
    A description of the takeoff type. Max length: 256
- `color` — `string`  
...
- `shapeType` — `enum:string`  
    The shape of the count marker. A count marker is one of the takeoff tool type options that is selected in the UI when creating a takeoff type. Possible values: CIRCLE, TRIANGLE, SQUARE, DIAMOND, CHECKMARK, RECTANGLE.
- `countMarkerSize` — `number`  
    The size of the count marker. Returns null when shapeType is RECTANGLE. For rectangles, use the height (countMarkerHeight) and width (countMarkerWidth) fields.
- `countMarkerWidth` — `number`  
    The width of the count marker. Applies only to rectangles (RECTANGLE) and returns null for other shape types. Use with countMarkerHeight to determine the full rectangle dimensions.
- `countMarkerHeight` — `number`  
    The height of the count marker. Applies only to rectangles (RECTANGLE) and returns null for other shape types. Use with countMarkerWidth to determine the full rectangle dimensions.
- `tool` — `enum:string`  
    The type of tool used to create takeoff items of this takeoff type. Possible values: COUNT, DISTANCE, AREA, SELECT. Corresponding UI names: COUNT, LINEAR, AREA, BIM.
- `propertyDefinitions` — `array: object`  
    A list of additional, user provided parameters associated with a 2D takeoff type.
  - `name` — `string`  
      The name of the property. Max length: 256
  ...
  - `valueLocation` — `enum:string`  
      The location of the value. For properties Length, Width, Height, Depth and Thickness: valueLocation is INSTANCE_WITH_TAKEOFF_TYPE_DEFAULT. For properties WeightByLength, WeightByArea and WeightByVolume: valueLocation is TAKEOFF_TYPE. Possible values: TAKEOFF_TYPE, INSTANCE_WITH_TAKEOFF_TYPE_DEFAULT
- `modelMappings` — `array: object`  
    A list of model properties used to calculate output. Only relevant for 3D models.
  - `name` — `string`  
      The name of the mapping. Max length: 256
  - `mappingExpression` — `string`  
      The mapping formula.
- `primaryQuantityDefinition` — `object`  
    The classification quantity details.
  - `outputName` — `string`  
  ...
  - `expression` — `string`  
      The formula to calculate the quantity.
  - `unitOfMeasure` — `enum:string`  
      The unit of measurement. Possible values: EA, IN, LF, YD, SI, SF, SY, CI, CF, CY, LBS, TON, MM, M, M2, M3, KG, T.
  - `classifications` — `array: object`  
      New. Classification assignments from the Classification Service that define what structures and nodes this quantity definition applies to.
    - `structureId` — `string: UUID`  
        The ID of the classification structure (tree). Classification trees are defined in the Classifications API. Use the value of the id field returned by GET trees/{treeId}.
    - `nodeId` — `string: UUID`  
        The node ID.
- `secondaryQuantityDefinitions` — `array: object`  
    A list of secondary classification quantity details.
  - `outputName` — `string`  
      A custom output name from the user.
  - `classificationCodeOne` — `string`  
      Deprecated. The classification code selected from the first classification system.
...
---
원본 문서: https://aps.autodesk.com/en/docs/acc/v1/reference/http/takeoff-projects-project_id-packages-package_id-takeoff-types-takeoff_type_id-GET
