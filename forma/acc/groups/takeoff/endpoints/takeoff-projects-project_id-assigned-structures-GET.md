---
operation_id: takeoff-projects-project_id-assigned-structures-GET
method: GET
path: /construction/takeoff/v1/projects/{projectId}/assigned-structures
group: "Takeoff"
...
# Retrieves the classification structures (trees) that have been assigned to a Takeoff project

```http
GET https://developer.api.autodesk.com/construction/takeoff/v1/projects/{projectId}/assigned-structures
```
...
Retrieves the classification structures (trees) that have been assigned to a Takeoff project. These structures are defined in the Classifications API.
...
| 이름 | 타입 | 필수 | 설명 |
| --- | --- | --- | --- |
| `offset` | int |  | The number of results to skip before returning data. This is zero-based. For example, offset=2 skips the first two results. |
| `limit` | int |  | The maximum number of assigned structures per page. Acceptable values: 1-5. Default value: 5. |
...
| 코드 | 의미 | 설명 |
| --- | --- | --- |
| `200` | OK | Successfully retrieved the project assigned structures. |
...
- `results` — `array: object`  
    A list of classification structures assigned to the project.
  ...
  - `createdAt` — `datetime: ISO 8601`  
      The date and time when the assigned structure was added to the project, in the following format: YYYY-MM-DDThh:mm:ssZ.
  - `createdBy` — `string`  
      The Autodesk ID of the user who added the assigned structure to the project. To find details about the user call GET users.

## Example

```
curl -v 'https://developer.api.autodesk.com/construction/takeoff/v1/projects/:projectId/assigned-structures?limit=2' \
...
```
{
  ...
  "results": [
    {
      "structureId": "4d5a4fd0-0fb8-11ed-aef7-0242ac120002",
...
---
원본 문서: https://aps.autodesk.com/en/docs/acc/v1/reference/http/takeoff-projects-project_id-assigned-structures-GET
