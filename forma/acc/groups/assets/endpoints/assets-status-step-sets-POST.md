---
operation_id: assets-status-step-sets-POST
method: POST
path: /construction/assets/v1/projects/{projectId}/status-step-sets
group: "Assets"
...
```http
POST https://developer.api.autodesk.com/construction/assets/v1/projects/{projectId}/status-step-sets
```
...
- `name` — `string` **(필수)**  
    The display name for the status set. This name appears in the Assets UI when viewing status sets. Max length: 100
- `description` — `string`  
    A description of the status set. Max length: 500
- `values` — `array: object` **(필수)**  
    An array of asset statuses to populate the new status set. This array defines new statuses.
  - `label` — `string` **(필수)**  
...
- `description` — `string`  
    A description of the status set. Max length: 500
- `isDefault` — `boolean`  
    Specifies if this is the status set that is assigned to categories by default - that is, this is the status set that will be assigned to categories if no other status set has been assigned, either explicitly or through inheritance. If true, it’s the default status set. If false, it’s not.
- `values` — `array: object`  
    An array of statuses that belong to this status set.
  - `label` — `string`  
      The display label of the status. This label appears in the Assets UI when viewing statuses. The label must be unique within this status set, but does not need to be unique within the project (in other words, the same label can be used in multiple status sets in the same project). “Uniqueness” in this context means case-insensitive.
  - `description` — `string`  
      A description of the status.
...
---
원본 문서: https://aps.autodesk.com/en/docs/acc/v1/reference/http/assets-status-step-sets-POST
