---
operation_id: assets-error-codes-GET
method: GET
path: /construction/assets/v1/error-codes
group: "Assets"
...
```http
GET https://developer.api.autodesk.com/construction/assets/v1/error-codes
```
...
| 코드 | 의미 | 설명 |
| --- | --- | --- |
| `200` | OK | Successfully retrieved all error codes. |
...
- `pagination` — `object`  
  ...
  - `limit` — `int`  
      Always equal to totalResults (Pagination is not used by this endpoint. This object is returned for backward compatibility.).
  - `offset` — `int`  
      Always equal to 0 (Pagination is not used by this endpoint. This object is returned for backward compatibility.).
  - `totalResults` — `int`  
...
---
원본 문서: https://aps.autodesk.com/en/docs/acc/v1/reference/http/assets-error-codes-GET
