---
operation_id: rfis-custom-attributes-attributeId-PATCH
method: PATCH
path: /construction/rfis/v3/projects/{projectId}/attributes/{attributeId}
group: "RFIs"
...
```http
PATCH https://developer.api.autodesk.com/construction/rfis/v3/projects/:projectId/attributes/:attributeId
```
...
- `possibleValues` — `object`  
  ...
  - `newAttributes` — `array: string,null`  
      Adds new possible values to the attribute. Each item is the name of a new possible value as shown in the UI.
  - `updatedAttributes` — `array: object`  
      Updates the names of existing possible values.
    - `id` — `string: UUID`  
  ...
  - `deletedAttributes` — `array: string`  
      Deletes possible values from the attribute. Each item is the ID of a possible value to delete.
...
---
원본 문서: https://aps.autodesk.com/en/docs/acc/v1/reference/http/rfis-custom-attributes-attributeId-PATCH
