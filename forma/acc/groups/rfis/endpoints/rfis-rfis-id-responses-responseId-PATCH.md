---
operation_id: rfis-rfis-id-responses-responseId-PATCH
method: PATCH
path: /construction/rfis/v3/projects/{projectId}/rfis/{rfiId}/responses/{responseId}
group: "RFIs"
...
```http
PATCH https://developer.api.autodesk.com/construction/rfis/v3/projects/:projectId/rfis/:rfiId/responses/:responseId
```
...
| `responseId` | string |  | The ID of the response. To find the ID, call GET rfis/:id. |
...
- `text` — `string` **(필수)**  
    The body text of the RFI response. This field supports plain text only.
- `onBehalf` — `object`  
    The Autodesk ID of the reviewer for whom the response is being submitted. To find user details, call GET users.’
- `status` — `enum:string` **(필수)**  
    The status of the response. To determine which status values the user is allowed to submit, call GET users/me. Possible values: answered, rejected.
- `attachments` — `array: object`  
    A list of files to attach to the RFI response.
  - `attachmentType` — `enum:string` **(필수)**  
      The type of the attachment. Will always be: rfiResponse
  - `displayName` — `string`  
      The user-facing name to show in the UI.
  - `fileName` — `string`  
      The internal file name.
  - `storageUrn` — `string` **(필수)**  
      The storage URN of the uploaded file. Use this value to generate a signed URL and download the file via the Data Management API. For more details, see the Submit RFI Response tutorial.
...
```
curl -v 'https://developer.api.autodesk.com/construction/rfis/v3/projects/:projectId/rfis/:rfiId/responses/:responseId' \
  -X 'PATCH' \
...
---
원본 문서: https://aps.autodesk.com/en/docs/acc/v1/reference/http/rfis-rfis-id-responses-responseId-PATCH
