---
operation_id: issues-issues-POST
method: POST
...
```http
POST https://developer.api.autodesk.com/construction/issues/v1/projects/{projectId}/issues
```
...
- `status` — `enum:string` **(필수)**  
    The current status of the issue. To check the available statuses for the project, call GET users/me and check the list of permitted statuses (issue.new.permittedStatuses). For more information about statuses, see the Help documentation. Possible values: draft, open, pending, in_progress, completed, in_review, not_approved, in_dispute, closed
- `assignedTo` — `string`  
...
- `customAttributes` — `array: object`  
  ...
  - `attributeDefinitionId` — `string: UUID` **(필수)**  
...
---
원본 문서: https://aps.autodesk.com/en/docs/acc/v1/reference/http/issues-issues-POST
