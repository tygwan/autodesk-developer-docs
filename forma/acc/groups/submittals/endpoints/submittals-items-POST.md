---
operation_id: submittals-items-POST
method: POST
path: /construction/submittals/v2/projects/{projectId}/items
group: "Submittals"
...
```http
POST https://developer.api.autodesk.com/construction/submittals/v2/projects/:projectId/items
```
...
- `customIdentifier` — `string`  
    A custom number to assign to the submittal item. Before creating an item, check what the custom numbering type is for the current project by calling GET metadata to get the custom identifier sequence type. Submittals support two custom numbering types: - Global Numbering: The custom identifier only contains the sequential number. - Spec Section Numbering: When the project uses spec section numbering for custom numbering, ensure that only the sequential number part of the custom identifier is sent in the request, excluding the spec ID. To find the next available custom number in the sequence, call GET items:next-custom-identifier. To verify whether the custom number has not been used and is in a supported format, call POST items:validate-custom-identifier. For more information on custom numbering, see the Help documentation.
- `typeId` — `string` **(필수)**  
    The UUID of the item type to assign to the submittal item, which can be obtained by calling GET item-types or GET item-types/:id.
- `specId` — `string: UUID` **(필수)**  
    The UUID of the spec section to assign to the submittal item.
- `subsection` — `string`  
    Define the subsection details of the submittal item. For example, 1.05-B.
- `title` — `string` **(필수)**  
    The title of the submittal item defined by the user when creating the submittal item.
- `description` — `string`  
    A description of the submittal item to add context about its purpose.
- `priority` — `enum:string`  
    The priority level of the submittal item. Possible values: Low, Normal, High. When the priority is not set, the default value is Normal.
- `stateId` — `enum:string` **(필수)**  
    The initial state of the submittal item, which depends on the role and permitted actions of the user. Possible values: draft, sbc-1, mgr-1. The required fields depend on the selected state ID. For example, if the state is mgr-1, required fields include subcontractor (subcontractor), subcontractor type (subcontractorType), and submitter due date (submitterDueDate). If the state is sbc-1, required fields include manager (manager) and manager type (manager-type). The required fields for creating the submittal item depend on the input stateId. The different states will require different mandatory fields. Refer Step 1: Verify User Permissions in the Create a Submittal Item tutorial for details.
- `manager` — `string`  
    The Autodesk ID (autodeskId) or Group ID (memberGroupId) specifying the manager to assign to the submittal item. The manager could be a user, role, or a company. Note that the user must be defined as a manager in the project. You can verify this by calling GET mappings. To get the possible values for a manager, call GET mappings and check the autodeskId. This returns a list of all the users, roles, and companies that are managers in the project. Call GET projects/users to verify the actual name of the user. If a company is a manager, call GET companies to verify the actual name of the company. Note that we do not currently support verifying names of roles.
- `managerType` — `enum:string`  
    The type of manager. This attribute is required when a manager is specified. To determine the type of manager, call GET mappings and check the user type. Possible values: 1 (user), 2 (company), 3 (role).
- `subcontractor` — `string`  
    The Autodesk ID of the subcontractor to assign to the submittal item. When creating a submittal item as a manager in the UI, you will see ‘assign to responsible contractor’. Get the subcontractor ID using: - GET projects/users to verify the actual name of the user in case the typs is a user (1). - GET companies to determine the name of the company in case the typs is a company (2). Note that we do not currently support verifying names of roles.
- `subcontractorType` — `enum:string`  
    The type of subcontractor. This attribute is required when a subcontractor is specified. Possible values: 1 (user), 2 (company), 3 (role).
...
- `submitterDueDate` — `string`  
    The date by which the subcontractor is expected to submit the submittal to the manager, in the following format: YYYY-MM-DD in UTC (ISO 8601). For example, 2018-02-15. This corresponds to the sbc-1 state Waiting for submission.
- `managerDueDate` — `string`  
    The date by which the manager is expected to prepare the submittal item for review to the reviewer, in the following format: YYYY-MM-DD in UTC (ISO 8601). For example, 2018-02-15. This corresponds to the mgr-1 state Open (Submitted).
- `packageId` — `string: UUID`  
    The ID of the package associated with the submittal item. To retrieve a list of packages and their corresponding IDs, call GET packages.
- `templateId` — `string: UUID`  
    The ID of a review template used to apply predefined steps and tasks when creating a submittal item. Templates must be created in the UI or retrieved by calling GET templates.

## 응답

| 코드 | 의미 | 설명 |
| --- | --- | --- |
| `201` | Created | Creation of a submittal item. |
...
- `manager` — `string`  
    The ID that was assigned to the manager of the submittal item. If the creator of the submittal item held a manager role, they were automatically set as the manager (manager). To determine the type of the manager (i.e., user, role, or company), refer to the manager type (managerType) attribute. In order to get more info about the manager, use: - GET projects/users to verify the actual name of the user in case the typs is a user (1). - GET companies to determine the name of the company in case the typs is a company (2). Note that we do not currently support verifying names of roles.
- `managerType` — `enum:string`  
...
```
{
  ...
  "createdAt": "2024-02-01T12:09:24.198466Z",
  "createdBy": "WD43ZJGKDFLFH",
  "updatedAt": "2024-02-01T12:09:24.198466Z",
...
---
원본 문서: https://aps.autodesk.com/en/docs/acc/v1/reference/http/submittals-items-POST
