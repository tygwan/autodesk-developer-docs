---
operation_id: submittals-specs-POST
method: POST
path: /construction/submittals/v2/projects/{projectId}/specs
group: "Submittals"
...
```http
POST https://developer.api.autodesk.com/construction/submittals/v2/projects/:projectId/specs
```
...
- `title` — `string` **(필수)**  
    The title of the spec section to be created.
- `identifier` — `string` **(필수)**  
    The unique ID to be assigned to the spec section within the UI. Ensure this ID is unique by checking existing spec sections using GET specs.
...
---
원본 문서: https://aps.autodesk.com/en/docs/acc/v1/reference/http/submittals-specs-POST
