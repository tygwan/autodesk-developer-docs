---
operation_id: submittals-async-jobs-asyncJobId-GET
method: GET
path: /construction/submittals/v2/projects/{projectId}/async-jobs/{asyncJobId}
group: "Submittals"
...
# Retrieves the current status and result of an asynchronous job

```http
GET https://developer.api.autodesk.com/construction/submittals/v2/projects/:projectId/async-jobs/:asyncJobId
```
...
Retrieves the current status and result of an asynchronous job. Poll this endpoint after triggering an operation that runs asynchronously, until the job reaches a terminal state (Success (3) or Failure (4)).
...
| `asyncJobId` | string: UUID |  | The ID of the async job. This ID is returned in the response of any endpoint that triggers an asynchronous operation. For example, POST custom-identifier:change-sequence-type. |
...
```
curl -v 'https://developer.api.autodesk.com/construction/submittals/v2/projects/9eae7d59-1469-4389-bfb2-4114e2ba5545/async-jobs/668b55f2-eb00-4308-bddf-b2f2e99e6692' \
...
---
원본 문서: https://aps.autodesk.com/en/docs/acc/v1/reference/http/submittals-async-jobs-asyncJobId-GET
