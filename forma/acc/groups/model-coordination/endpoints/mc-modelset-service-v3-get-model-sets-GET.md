---
operation_id: mc-modelset-service-v3-get-model-sets-GET
method: GET
path: /bim360/modelset/v3/containers/{containerId}/modelsets
...
- `modelSets` — `array: object`  
    List of model set summaries. Max items: 20.
  - `modifiedBy` — `string`  
...
```
curl -v 'https://developer.api.autodesk.com/bim360/modelset/v3/containers/f0f4f36a-ac64-687f-b132-8efe04b22454/modelsets?pageLimit=134&continuationToken=10&name=Formal+Coordination&folderUrn=urn%3aadsk.wipprod%3afs.folder%3aco.WI8roO18TU2Cl3P9y64z4w&includeDisabled=False&includeDeleted=True' \
     -H 'Authorization: Bearer <token>'
```
...
- `pageLimit` — `int32`  
    The maximum number of model sets to be returned in a single page. If not set, the default page limit is used, as determined by the server. If set to higher than the server’s maximum limit then the server’s limit will be used.
- `continuationToken` — `string`  
...
- `includeDeleted` — `boolean`  
    Determines whether to include deleted model sets.
...
---
원본 문서: https://aps.autodesk.com/en/docs/acc/v1/reference/http/mc-modelset-service-v3-get-model-sets-GET
