---
operation_id: mc-modelset-service-v3-search-model-set-view-lineages-POST
method: POST
path: /bim360/modelset/v3/containers/{containerId}/modelsets/{modelSetId}/views:lineages
...
# Retrieves a list of model set views in a given model set that contain the provided set of document lineage URNs

```http
POST https://developer.api.autodesk.com/bim360/modelset/v3/containers/:containerId/modelsets/:modelSetId/views:lineages
```
...
- `array: string` — `array: string` **(필수)**  
    An array of lineage URNs. Min items: 1. Max items: 1000.
...
- `modelSetViews` — `array: object`  
    The list of model set views. Max items: 20.
  - `name` — `string`  
  ...
  - `definition` — `array: object`  
      The definition of models in a model set view, which is used to track the same models through time. Min items: 1. Max items: 50.
    - `lineageUrn` — `string`  
        The lineage URN of the seed file or document lineage to track with this view. Min length: 1. Max length: 256.
    - `viewableName` — `string`  
        The name of the viewable in the Model Derivative manifest to track along the seed file lineage. This value is ignored if lineageUrn is the URN of a BIM 360 Docs Plans folder document. Min length: 1. Max length: 430.
  - `modifiedBy` — `string`  
  ...
  - `published` — `object`  
      If this view is published, the data concerning the publish destination.
    - `name` — `string`  
        The name of the seed file representing the published view. Min length: 1. Max length: 64.
    - `time` — `datetime: ISO 8601`  
        The time of the last publish operation.
    - `folderUrn` — `string`  
        The URN of the folder in which the published view resides.
    - `versionUrn` — `string`  
        The URN of the seed file representing the published view.
...
```
curl -v 'https://developer.api.autodesk.com/bim360/modelset/v3/containers/f0f4f36a-ac64-687f-b132-8efe04b22454/modelsets/00fb28a5-e8a4-2755-562a-7c2f0fc87911/views:lineages' \
     -X POST \
     -H 'Authorization: Bearer <token>' \
     -H 'Content-Type: application/json' \
     -d '[
           "urn:adsk.wipprod:dm.lineage:jvMF7mrHR7OwG_DToKsJUA"
         ]'
```
...
```
curl -v 'https://developer.api.autodesk.com/bim360/modelset/v3/containers/f0f4f36a-ac64-687f-b132-8efe04b22454/modelsets/00fb28a5-e8a4-2755-562a-7c2f0fc87911/views:lineages?exact=False&pageLimit=134&continuationToken=10' \
     -X POST \
     -H 'Authorization: Bearer <token>' \
     -H 'Content-Type: application/json' \
     -d '[
...
- `exact` — `boolean`  
    Determines whether to exactly match the set of lineages passed.
- `pageLimit` — `int32`  
    The maximum number of views to be returned in a single page. If not set, the default page limit is used, as determined by the server. If set to higher than the server’s maximum limit then the server’s limit will be used.
- `continuationToken` — `string`  
...
---
원본 문서: https://aps.autodesk.com/en/docs/acc/v1/reference/http/mc-modelset-service-v3-search-model-set-view-lineages-POST
