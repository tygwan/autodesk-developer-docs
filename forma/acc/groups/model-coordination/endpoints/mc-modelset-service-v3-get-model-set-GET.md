---
operation_id: mc-modelset-service-v3-get-model-set-GET
method: GET
...
- `modifiedBy` — `string`  
    The unique identifier of the user who modified the model set.
- `modifiedTime` — `datetime: ISO 8601`  
    The date and time that the model set was modified.
- `modelSetId` — `string: UUID`  
    The GUID that uniquely identifies the model set.
- `modelSetType` — `enum: string`  
...
- `rootFolder` — `string`  
    The URN of the root folder for this model set.
- `folders` — `array: object`  
    The ModelSetFolders that determine the documents included in the model set.
  - `folderUrn` — `string`  
      The ID of the folder in your project (can be found using the Data Management API).
- `includedFolders` — `array: object`  
    The sub-folders included in this model set, as determined by any defined content filters.
  - `folderUrn` — `string`  
      The ID of the folder in your project (can be found using the Data Management API).
  - `folderName` — `string`  
      The name of the folder in your project.
  - `parentFolderUrn` — `string`  
      The ID of the parent folder in your project.
- `name` — `string`  
...
- `accessedTime` — `datetime: ISO 8601`  
    The date and time that the model set was last accessed.
- `isInactive` — `boolean`  
    Indicates that versions of this model set have not been accessed recently.
- `isDisabled` — `boolean`  
    If set to true, a model set version is not automatically created following changes to documents within the model set.
- `isDeleted` — `boolean`  
    Indicates whether the model set is deleted.
- `tipVersion` — `int32`  
    The version number of the most recent version of the model set.
- `clashEngineVersion` — `int32`  
    Indicates the version of the Clash engine used to process this model set.
- `permission` — `enum: string`  
    An action that can be performed on a model set. Possible values: Edit, Read.
- `contentFilters` — `object`  
    Filters to apply to the content in your Coordination Space to exclude models by folder or other data.
  - `explicitFolders` — `array: string`  
      An explicit list of folder IDs to include in the Coordination Space (can be found using the Data Management API). Max items: 100.
  - `recursiveFolders` — `array: string`  
      A list of recursive folder IDs to include in the Coordination Space (can be found using the Data Management API). Max items: 100.
  - `globalFileFormat` — `array: string`  
      A list of file extensions that represent the global file format filter. Min items: 1. Max items: 25.
  - `fileExtensions` — `object`  
      A dictionary of explicit or recursive folder URNs mapped to a file extension object that specifies the chosen file extensions for a given set of folder URNs.
- `checksum` — `string`  
    An opaque string describing the current state of the object, used to prevent update collisions.
- `hasContentFilters` — `boolean`  
    Indicates if the model set content has been filtered.
- `includedFolderCount` — `int`  
    The number of folders included in this model set, refreshed every time the model set is accessed directly.
- `isDocumentLimitReached` — `boolean`  
    Indicates whether the model set is currently blocked from creating new versions because it has reached the documents limit.
...
```
curl -v 'https://developer.api.autodesk.com/bim360/modelset/v3/containers/f0f4f36a-ac64-687f-b132-8efe04b22454/modelsets/00fb28a5-e8a4-2755-562a-7c2f0fc87911?showIncludedFolders=True' \
     -H 'Authorization: Bearer <token>'
```

```
{
  "modifiedBy": "PD23PXGV8V3V",
  "modelSetId": "00fb28a5-e8a4-2755-562a-7c2f0fc87911",
  "modelSetType": "Plans",
  "containerId": "f0f4f36a-ac64-687f-b132-8efe04b22454",
  "rootFolder": "urn:adsk.wipprod:fs.folder:co.WI8roO18TU2Cl3P9y64z4w",
  "folders": [
    {
  ...
  "createdTime": "2015-10-21T16:31:44.000Z",
  "accessedTime": "2015-10-21T16:33:02.000Z",
  "isInactive": true,
  "isDisabled": true,
  "isDeleted": false,
  "tipVersion": 120,
  ...
  "contentFilters": {
    "explicitFolders": [
      "urn:adsk.wipprod:fs.folder:co.WI8roO18TU2Cl3P9y64z4w"
    ],
    "recursiveFolders": [
      "urn:adsk.wipprod:fs.folder:co.WI8roO18TU2Cl3P9y64z4w"
    ]
  },
  "checksum": "7215ee9c7d9dc229d2921a40e899ec5f",
  "hasContentFilters": true,
  "includedFolderCount": 233,
  "isDocumentLimitReached": true
}
```
...
- `showIncludedFolders` — `boolean`  
    Whether to populate the included folders array. Set to false if the array is not required. Defaults to true.
...
---
원본 문서: https://aps.autodesk.com/en/docs/acc/v1/reference/http/mc-modelset-service-v3-get-model-set-GET
