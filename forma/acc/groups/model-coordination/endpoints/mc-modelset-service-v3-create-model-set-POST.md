---
operation_id: mc-modelset-service-v3-create-model-set-POST
method: POST
...
Creates a model set within a given container specifying the folder used to determine the set of model document lineages comprising the model set. Currently only a single folder is supported; however, sub-folders are supported. The response contains information about the created model set job.
...
- `contentFilters` — `object`  
  ...
  - `explicitFolders` — `array: string` **(필수)**  
  ...
  - `recursiveFolders` — `array: string` **(필수)**  
...
| `409` | Conflict | Limit exceeded for enabled model sets, create a disabled model set or disable an existing model set, or a model set with the same name already exists. |
...
---
원본 문서: https://aps.autodesk.com/en/docs/acc/v1/reference/http/mc-modelset-service-v3-create-model-set-POST
