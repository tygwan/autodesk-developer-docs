---
operation_id: relationship-service-v2-relationships-sync-POST
method: POST
path: /bim360/relationship/v2/containers/{containerId}/relationships:sync
group: "Relationships"
...
- `filters` — `object`  
  ...
  - `domains` — `array: string`  
      An array of domains on which to filter. Min items: 1 Max items: 20
...
- `current` — `object`  
  ...
  - `data` — `array: object`  
      A set of current relationships relative to the sync token passed by the caller.
    - `id` — `string: UUID`  
...
- `deleted` — `object`  
  ...
  - `data` — `array: object`  
      A set of relationships that have been deleted relative to the sync token passed by the caller.
    - `id` — `string: UUID`  
        The UUID that uniquely identifies the relationship.
    - `createdOn` — `datetime: ISO 8601`  
        The date and time the relationship was created.
...
---
원본 문서: https://aps.autodesk.com/en/docs/acc/v1/reference/http/relationship-service-v2-relationships-sync-POST
