---
operation_id: cost-cost-items-costItemId-PATCH
method: PATCH
...
```http
PATCH https://developer.api.autodesk.com/cost/v1/containers/:containerId/cost-items/:costItemId
```
...
| `costItemId` | string |  | The cost item ID. |
...
- `name` — `string`  
    Name of the cost item. Max length: 1024
- `description` — `string`  
    Detail description of the cost item. Max length: 2048
- `type` — `string,null`  
...
- `proposedExchangeRate` — `number,string,null`  
    The exchange rate for the proposed cost. If multi-currency is not enabled, this value is set to 1 regardless of what you specify here. Default value: 1.
...
---
원본 문서: https://aps.autodesk.com/en/docs/acc/v1/reference/http/cost-cost-items-costItemId-PATCH
