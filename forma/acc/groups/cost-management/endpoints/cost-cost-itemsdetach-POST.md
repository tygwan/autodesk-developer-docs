---
operation_id: cost-cost-itemsdetach-POST
method: POST
path: /cost/v1/containers/{containerId}/cost-items:detach
...
```http
POST https://developer.api.autodesk.com/cost/v1/containers/:containerId/cost-items:detach
```
...
- `changeOrderId` — `string: UUID` **(필수)**  
    The ID of the change order to which the cost item is attached.
- `costItemId` — `string: UUID` **(필수)**  
    The ID of the cost item to detach.
...
```
curl -v 'https://developer.api.autodesk.com/cost/v1/containers/e94b9bc8-1775-4d76-9b1d-c613e120ccff/cost-items:detach' \
  -X 'POST' \
...
---
원본 문서: https://aps.autodesk.com/en/docs/acc/v1/reference/http/cost-cost-itemsdetach-POST
