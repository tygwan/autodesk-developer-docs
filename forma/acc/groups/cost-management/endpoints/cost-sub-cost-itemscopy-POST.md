---
operation_id: cost-sub-cost-itemscopy-POST
method: POST
path: /cost/v1/containers/{containerId}/cost-items/{costItemId}/sub-cost-items:copy
...
```http
POST https://developer.api.autodesk.com/cost/v1/containers/:containerId/cost-items/:costItemId/sub-cost-items:copy
```
...
- `from` — `enum:string` **(필수)**  
    The type of the sub cost items being copied. Note that contract and budget can only be used as a source type, not a destination. Possible values: contract, budget, estimated, proposed, submitted, approved, committed.
- `to` — `enum:string` **(필수)**  
    The type to which the sub cost items are copied. Note that contract and budget cannot be used as a destination. Possible values: estimated, proposed, submitted, approved, committed.
- `source` — `object`  
  ...
  - `ids` — `array: string`  
      A list of budget IDs or Schedule of Value (SOV) IDs from a contract, used as the source for copying sub cost items. To retrieve the relevant IDs, call GET budgets or GET contracts.
...
---
원본 문서: https://aps.autodesk.com/en/docs/acc/v1/reference/http/cost-sub-cost-itemscopy-POST
