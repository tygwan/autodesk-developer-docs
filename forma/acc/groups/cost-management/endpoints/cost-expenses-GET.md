---
operation_id: cost-expenses-GET
method: GET
path: /cost/v1/containers/{containerId}/expenses
...
```http
GET https://developer.api.autodesk.com/cost/v1/containers/:containerId/expenses
```
...
- `results` — `array: object`
  ...
  - `supplierCompanyUid` — `string,null`  
  ...
  - `supplierName` — `string,null`  
      The supplier company name for the expense. Any company can be used regardless the ones added in Hub Administration. Max length: 255
  - `mainContractId` — `string`  
      The ID of the Main Contract to which the expense belongs.
  - `budgetPaymentId` — `string,null`  
      The ID of the Budget Pay App the expense amount will be aggregated to.
  - `number` — `string`  
      The auto-generated sequence number for the expense. Max length: 255
  - `name` — `string`  
      The name of the expense. Max length: 1024
  - `description` — `string`  
      The detailed description of the expense. Max length: 2048
  - `note` — `string`  
      The note attached to the expense, if returned. The note consists of Tiptap formatted rich text. For more information, see https://tiptap.dev.
  - `term` — `string`  
  ...
  - `type` — `string,null`  
      The type of the expense. This is customizable by the project administrator. Max length: 128
  - `scope` — `string,null`  
  ...
  - `amount` — `number,string,null`  
      The total value of the subitems of the expense.
  - `paymentDue` — `string,null`  
      The payment due date and time of the expense.
  ...
  - `forecastDistributionAt` — `string,null`  
      The date and time determine which distribution period this expense belongs to, after the status of status is set to approved or paid.
...
---
원본 문서: https://aps.autodesk.com/en/docs/acc/v1/reference/http/cost-expenses-GET
