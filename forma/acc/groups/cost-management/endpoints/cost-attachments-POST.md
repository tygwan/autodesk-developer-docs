---
operation_id: cost-attachments-POST
method: POST
...
```http
POST https://developer.api.autodesk.com/cost/v1/containers/:containerId/attachments
```
...
- `name` — `string` **(필수)**  
    The name of the attachment. Max length: 255
- `folderId` — `string: UUID`  
    The folder ID retrieved from attachment-folder.
- `urn` — `string`  
    The version URN from BIM 360 Forma Data Management after the attachment is uploaded.
- `templateId` — `string`  
    The documentTemplate Id to generate an attachment
- `documentPackageId` — `string,null`  
    The ID of the document package
- `associationId` — `string: UUID` **(필수)**  
    The object ID of the item associated with the actions, such as a budget, contract, or cost item.
- `associationType` — `enum:string` **(필수)**  
...
curl -v 'https://developer.api.autodesk.com/cost/v1/containers/e94b9bc8-1775-4d76-9b1d-c613e120ccff/attachments' \
  ...
  -d '{
        ...
        "documentPackageId": "18d97ae0-9484-11e8-a7ec-7ddae203e404",
...
---
원본 문서: https://aps.autodesk.com/en/docs/acc/v1/reference/http/cost-attachments-POST
