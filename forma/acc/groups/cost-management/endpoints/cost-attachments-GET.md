---
operation_id: cost-attachments-GET
method: GET
path: /cost/v1/containers/{containerId}/attachments
...
# Retrieves all of the attachments associated with an item such as a budget, contract, or cost item

```http
GET https://developer.api.autodesk.com/cost/v1/containers/:containerId/attachments
```
...
| `sort` | string |  | Field to sort attachment results by. Only fields stored at the attachment database level are supported. Allowed values: name, position, createdAt, updatedAt. Each attribute can be sorted in asc (default) or desc order. For example, sort=name desc. |
...
- `results` — `array: object`  
  ...
  - `id` — `string: UUID`  
      The unique auto-generated identifier of the attachment.
  - `folderId` — `string: UUID`  
      The unique identifier (UUID) of the folder named attachment-folder where the attachment is stored. This ID is automatically retrieved by the backend and returned in the response.
  - `urn` — `string`  
      The object version URN of the attachment in the Autodesk BIM 360 Forma Data Management service.
  - `pdfOssUrn` — `string,null`  
      Not relevant
  - `pdfUrn` — `string,null`  
      Not relevant
  - `type` — `enum:string`  
      The type of attachment. Possible values: Upload: is a locally uploaded file. DocsFile is a file referenced from BIM 360 Forma Data Management. Reference is a file referenced form report. Document is for document generation. Max length: 64
  - `name` — `string`  
      The name of the attachment. Max length: 1024
  - `replaceIfExists` — `boolean`  
      Determines whether an existing attachment with a matching name should be replaced.
  - `status` — `string`  
      The status of the attachment. Max length: 1024
  - `documentPackageId` — `string,null`  
      Not relevant
  - `associationId` — `string: UUID`  
      The object ID of the item associated with the actions, such as a budget, contract, or cost item.
  - `associationType` — `enum:string`  
      The type of item to which it is associated. Possible values: Budget, Contract, ScheduleOfValue, FormInstance, CostItem, Payment, MainContract, BudgetPayment, Expense, CostPayment, ExpenseItem, PaymentItem, OCO, RCO, SCO, PCO, RFQ, DistributionItem, BudgetTransfer, Fee
...
```
curl -v 'https://developer.api.autodesk.com/cost/v1/containers/e94b9bc8-1775-4d76-9b1d-c613e120ccff/attachments?filter[associationType]=Budget&filter[lastModifiedSince]=2020-03-01T13:00:00Z&limit=100&sort=name' \
  ...
  "results": [
    {
      ...
      "status": "Pending",
      "documentPackageId": "3A7432BB-8CA5-444A-B327-EE010E1D89DF",
...
---
원본 문서: https://aps.autodesk.com/en/docs/acc/v1/reference/http/cost-attachments-GET
