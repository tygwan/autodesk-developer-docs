---
operation_id: cost-change-orders-changeOrder-GET
method: GET
path: /cost/v1/containers/{containerId}/change-orders/{changeOrder}
...
```http
GET https://developer.api.autodesk.com/cost/v1/containers/:containerId/change-orders/:changeOrder
```
...
| `sort` | string |  | A field to sort results by. Only fields stored at the change order database level are supported. Possible values: name, number, createdAt, updatedAt, statusChangedAt. Each attribute can be sorted in asc (default) or desc order. For example, sort=name desc. |
...
```
curl -v 'https://developer.api.autodesk.com/cost/v1/containers/e94b9bc8-1775-4d76-9b1d-c613e120ccff/change-orders/pco?limit=100&sort=name&filter[sourceId]=b6445638-ca68-4e3c-9160-15864de6b818&include=include=costItems,attributes,comments&filter[budgetStatus]=open&filter[costStatus]=open&filter[lastModifiedSince]=2020-03-01T13:00:00Z' \
...
---
원본 문서: https://aps.autodesk.com/en/docs/acc/v1/reference/http/cost-change-orders-changeOrder-GET
