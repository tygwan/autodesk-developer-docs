---
operation_id: cost-expenses-id-PATCH
method: PATCH
...
```http
PATCH https://developer.api.autodesk.com/cost/v1/containers/:containerId/expenses/:id
```
...
| `id` | array: string |  | The unique ID of the expense. You can obtain this ID from the response to the POST expenses or GET expenses endpoint. |
...
- `supplierName` — `string,null`  
    The supplier name for the expense. User can create a new supplier or select from BIM 360 Admin. Max length: 255
- `number` — `string`  
    The auto-generated sequence number for the expense. Max length: 255
- `name` — `string`  
    A name for the expense. Max length: 1024
- `description` — `string`  
    A detailed description of the expense. Max length: 2048
- `note` — `string`  
...
---
원본 문서: https://aps.autodesk.com/en/docs/acc/v1/reference/http/cost-expenses-id-PATCH
