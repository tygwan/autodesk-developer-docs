---
operation_id: cost-change-orders-changeOrder-POST
method: POST
...
```http
POST https://developer.api.autodesk.com/cost/v1/containers/:containerId/change-orders/:changeOrder
```
...
- `name` — `string`  
    Name of the change order Max length: 1024
- `description` — `string`  
    Detailed description of the change order. Max length: 2048
- `scope` — `enum:string`  
    The scope of the change order. Possible values: out, in, tbd , budgetOnly, contingency.
- `scopeOfWork` — `string`  
    Scope of work of the change order. This is a Tiptap formatted rich text (https://tiptap.dev/introduction/).
- `note` — `string`  
...
---
원본 문서: https://aps.autodesk.com/en/docs/acc/v1/reference/http/cost-change-orders-changeOrder-POST
