---
operation_id: cost-expenses-expenseId-items-POST
method: POST
...
```http
POST https://developer.api.autodesk.com/cost/v1/containers/:containerId/expenses/:expenseId/items
```
...
- `number` — `string`  
    The sequence number for the expense item, which will be auto-generated if it is not passed Max length: 255
- `name` — `string` **(필수)**  
...
---
원본 문서: https://aps.autodesk.com/en/docs/acc/v1/reference/http/cost-expenses-expenseId-items-POST
