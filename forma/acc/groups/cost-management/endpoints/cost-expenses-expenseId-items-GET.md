---
operation_id: cost-expenses-expenseId-items-GET
method: GET
path: /cost/v1/containers/{containerId}/expenses/{expenseId}/items
...
```http
GET https://developer.api.autodesk.com/cost/v1/containers/:containerId/expenses/:expenseId/items
```
...
| `expenseId` | array: string |  | A list of the object IDs of the expenses whose items you want to retrieve, separated by commas. You can obtain these IDs from the response to the POST expenses or GET expenses endpoint. |

### 쿼리 파라미터

| 이름 | 타입 | 필수 | 설명 |
| --- | --- | --- | --- |
| `include` | array: string |  | A list of the nested expense resources to include in the response with the expense items. For example, include=budget returns the IDs of budgets related to the expense items, include=attributes will return custom attributes which represents the “properties” in the response. Possible values: budget, contract, attributes, externalRelationship. |
...
- `results` — `array: object`
  ...
  - `name` — `string`  
      The name of the expense item. Max length: 1024
  - `description` — `string`  
      The detailed description of the expense item. Max length: 2048
  - `note` — `string`  
...
---
원본 문서: https://aps.autodesk.com/en/docs/acc/v1/reference/http/cost-expenses-expenseId-items-GET
