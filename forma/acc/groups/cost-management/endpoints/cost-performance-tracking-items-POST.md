---
operation_id: cost-performance-tracking-items-POST
method: POST
path: /cost/v1/containers/{containerId}/performance-tracking-items
...
# Creates a performance tracking item from the specified budget in the given project

```http
POST https://developer.api.autodesk.com/cost/v1/containers/:containerId/performance-tracking-items
```
...
Creates a performance tracking item from the specified budget in the given project. The tracking item’s attributes are a subset of the budget’s attributes. Note that you can create only one tracking item per budget.
...
- `budgetId` — `string: UUID` **(필수)**  
    The unique ID of the budget from which to create the performance tracking item. To find the budget ID, call GET budgets and inspect results.id in the response.
...
- `containerId` — `string: UUID`  
    The ID of the cost container for the project to which this tracking item belongs.
- `budgetId` — `string: UUID`  
    The ID of the tracking item’s underlying budget.
- `name` — `string`  
    The name of the tracking item’s underlying budget.
- `description` — `string`  
    The description of the tracking item’s underlying budget.
- `code` — `string`  
    The code of the tracking item’s underlying budget.
- `quantity` — `number`  
    The quantity of the performance tracking item’s underlying budget.
- `unit` — `string`  
    The unit of the performance tracking item’s underlying budget.
- `unitPrice` — `number,string,null`  
    The unit price of the performance tracking item’s underlying budget.
- `originalAmount` — `number`  
    Original amount of the budget, equals to quantity * unitPrice
- `plannedStartDate` — `datetime: ISO 8601`  
    The planned start date of the performance tracking item’s underlying budget.
- `plannedEndDate` — `datetime: ISO 8601`  
    The planned end date of the performance tracking item’s underlying budget.
...
```
curl -v 'https://developer.api.autodesk.com/cost/v1/containers/e94b9bc8-1775-4d76-9b1d-c613e120ccff/performance-tracking-items' \
  -X 'POST' \
...
---
원본 문서: https://aps.autodesk.com/en/docs/acc/v1/reference/http/cost-performance-tracking-items-POST
