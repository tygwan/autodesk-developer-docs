---
operation_id: cost-property-valuesbatch-update-POST
method: POST
path: /cost/v1/containers/{containerId}/property-values:batch-update
...
```http
POST https://developer.api.autodesk.com/cost/v1/containers/:containerId/property-values:batch-update
```
...
- `propertyDefinitionId` — `string: UUID` **(필수)**  
    ID of the custom attribute definition.
- `value` — `string,null,number,boolean` **(필수)**  
...
---
원본 문서: https://aps.autodesk.com/en/docs/acc/v1/reference/http/cost-property-valuesbatch-update-POST
