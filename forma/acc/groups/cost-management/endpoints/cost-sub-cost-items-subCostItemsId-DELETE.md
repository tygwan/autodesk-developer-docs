---
operation_id: cost-sub-cost-items-subCostItemsId-DELETE
method: DELETE
...
```http
DELETE https://developer.api.autodesk.com/cost/v1/containers/:containerId/cost-items/:costItemId/sub-cost-items/:subCostItemsId
```
...
| `subCostItemsId` | string: UUID |  | The ID of the sub cost item to be deleted. Note that only one sub cost item can be deleted per request. To find the sub cost item ID, call GET sub-cost-items. |
...
---
원본 문서: https://aps.autodesk.com/en/docs/acc/v1/reference/http/cost-sub-cost-items-subCostItemsId-DELETE
