---
operation_id: cost-segments-segmentId-PATCH
method: PATCH
path: /cost/v1/containers/{containerId}/templates/{templateId}/segments/{segmentId}
...
```http
PATCH https://developer.api.autodesk.com/cost/v1/containers/:containerId/templates/:templateId/segments/:segmentId
```
...
| `segmentId` | string: UUID |  | The segment ID. To obtain a segment ID, use GET templates/:templateId/segments. |

### 쿼리 파라미터

| 이름 | 타입 | 필수 | 설명 |
| --- | --- | --- | --- |
| `force` | boolean |  | Force delete segment values when the segment length changes. true: force delete, false: not force delete. |
...
- `isLocked` — `boolean,null`  
    true Lock the segment. false Do not lock the segment.
...
---
원본 문서: https://aps.autodesk.com/en/docs/acc/v1/reference/http/cost-segments-segmentId-PATCH
