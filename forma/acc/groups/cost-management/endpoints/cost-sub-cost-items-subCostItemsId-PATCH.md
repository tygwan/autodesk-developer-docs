---
operation_id: cost-sub-cost-items-subCostItemsId-PATCH
method: PATCH
path: /cost/v1/containers/{containerId}/cost-items/{costItemId}/sub-cost-items/{subCostItemsId}
...
```http
PATCH https://developer.api.autodesk.com/cost/v1/containers/:containerId/cost-items/:costItemId/sub-cost-items/:subCostItemsId
```
...
| `subCostItemsId` | string: UUID |  | The ID of the sub cost item to be updated. To find the sub cost item ID, call GET sub-cost-items. |
...
- `code` — `string`  
    A user-defined identifier for the sub cost item, used for categorization or identification. Max length: 255
- `name` — `string`  
    The name to give to the sub cost item. Max length: 1024
- `lockedField` — `string,null`  
    The field that is locked for cost calculations. This ensures the locked value remains unchanged during updates. Possible values: value, quantity, unitPrice. For example, if you set unitPrice at $100, you can adjust the quantity while keeping the cost fixed.
...
| 코드 | 의미 | 설명 |
| --- | --- | --- |
| `200` | OK | The sub cost item was successfully updated. |
...
---
원본 문서: https://aps.autodesk.com/en/docs/acc/v1/reference/http/cost-sub-cost-items-subCostItemsId-PATCH
