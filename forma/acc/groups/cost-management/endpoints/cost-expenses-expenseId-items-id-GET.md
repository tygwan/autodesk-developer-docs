---
operation_id: cost-expenses-expenseId-items-id-GET
method: GET
path: /cost/v1/containers/{containerId}/expenses/{expenseId}/items/{id}
...
```http
GET https://developer.api.autodesk.com/cost/v1/containers/:containerId/expenses/:expenseId/items/:id
```
...
| `expenseId` | string: UUID |  | The unique ID of the expense to which the new expense item belongs. You can obtain this ID from the response to the POST expenses or GET expenses endpoint. |
| `id` | string: UUID |  | The unique ID of the expense item. You can obtain this ID from the response to the POST expenses/:expenseId/items or GET expenses/:expenseId/items endpoint. |
...
---
원본 문서: https://aps.autodesk.com/en/docs/acc/v1/reference/http/cost-expenses-expenseId-items-id-GET
