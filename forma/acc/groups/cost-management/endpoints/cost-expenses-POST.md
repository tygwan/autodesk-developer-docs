---
operation_id: cost-expenses-POST
method: POST
...
```http
POST https://developer.api.autodesk.com/cost/v1/containers/:containerId/expenses
```
...
- `supplierName` — `string,null`  
    The supplier company name for the expense. Any company can be used regardless the ones added in Hub Administration. Required if supplierId is not set. Max length: 255
- `contractId` — `string,null`  
    The ID of the contract used to create expense items. Expense items are created based on the contract’s root schedule of values.
- `number` — `string`  
...
---
원본 문서: https://aps.autodesk.com/en/docs/acc/v1/reference/http/cost-expenses-POST
