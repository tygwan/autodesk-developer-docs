---
operation_id: cost-change-orders-GET
method: GET
path: /cost/v1/containers/{containerId}/change-orders
...
```http
GET https://developer.api.autodesk.com/cost/v1/containers/:containerId/change-orders
```
...
- `id` — `string: UUID`  
    Unique auto-generated identifier of the change order type.
- `name` — `string`  
    The name of the change order type.
- `type` — `string`  
    The type of the change order type.
- `actAs` — `string`  
    The status and rules of the change order.
- `position` — `int`  
    Not relevant
- `workflowType` — `string`  
    Not relevant
...
---
원본 문서: https://aps.autodesk.com/en/docs/acc/v1/reference/http/cost-change-orders-GET
