---
operation_id: cost-schedule-of-values-POST
method: POST
...
```http
POST https://developer.api.autodesk.com/cost/v1/containers/:containerId/schedule-of-values
```
...
- `parentId` — `string,null` **(필수)**  
    The ID of the SOV item’s parent item. For root SOV items, this value is null. Root items can only be created when allocating a budget to a contract.
- `contractId` — `string`  
...
---
원본 문서: https://aps.autodesk.com/en/docs/acc/v1/reference/http/cost-schedule-of-values-POST
