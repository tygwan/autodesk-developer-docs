---
operation_id: cost-main-contracts-mainContractId-items-id-PATCH
method: PATCH
...
```http
PATCH https://developer.api.autodesk.com/cost/v1/containers/:containerId/main-contracts/:mainContractId/items/:id
```
...
- `code` — `string`  
    A code for the main contract item. Max length: 255
- `name` — `string`  
    A name for the main contract item. Max length: 1024
- `description` — `string`  
...
- `lockedField` — `string,null`  
    The name of the main contract item field to lock so the API can use it to calculate the value of a related field. For example, you can lock the unitPrice field in the existing main contract item, and the API will use the new value of quantity that you provide to calculate the new value of amount. Possible values: amount, quantity, and unitPrice.
...
---
원본 문서: https://aps.autodesk.com/en/docs/acc/v1/reference/http/cost-main-contracts-mainContractId-items-id-PATCH
