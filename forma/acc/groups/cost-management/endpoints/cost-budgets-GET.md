---
operation_id: cost-budgets-GET
method: GET
path: /cost/v1/containers/{containerId}/budgets
...
```http
GET https://developer.api.autodesk.com/cost/v1/containers/:containerId/budgets
```
...
- `results` — `array: object`  
  ...
  - `code` — `string`  
      Unique code compliant with the budget code template defined by the project admin. Max length: 255
  - `scope` — `enum:string`  
      The scope of the budget. Possible values are budgetOnly and budgetAndCost.
  - `subItems` — `array,null`  
      The sub-budget in the root budget. Max length: 255
  - `budgetCode` — `object,null`  
      Budget code template and budget segment codes used in the budget. Not applicable for sub-budget.
  - `codeSegmentValues` — `object`  
      Not relevant
  - `name` — `string`  
      The name of the budget. Max length: 1024
  - `description` — `string`  
      The detail description of the budget. Max length: 2048
  - `quantity` — `number`  
  ...
  - `mainContractId` — `string: UUID`  
      Not relevant
  - `mainContractAmountSource` — `enum:string`  
      Not relevant
  - `isMarkup` — `boolean`  
      Not relevant
  - `contractIds` — `array: string`  
      Not relevant
  - `costUnitPrice` — `number,string,null`  
      Not relevant
...
---
원본 문서: https://aps.autodesk.com/en/docs/acc/v1/reference/http/cost-budgets-GET
