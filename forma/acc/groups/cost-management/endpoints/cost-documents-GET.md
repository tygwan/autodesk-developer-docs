---
operation_id: cost-documents-GET
method: GET
path: /cost/v1/containers/{containerId}/documents
...
```http
GET https://developer.api.autodesk.com/cost/v1/containers/:containerId/documents
```
...
| `associationType` | enum:string | **필수** | The type of the item is associated to. Possible values Budget, Contract, CostItem, FormInstance, Payment, BudgetPayment, Expense, OCO, SCO. |
| `filter[latest]` | boolean |  | Filter to get only the latest version of a document if it has been generated multiple times. For example, filter[latest]=true. This is deprecated as we’ll always response the latest document. |
...
- `id` — `string: UUID`  
    Unique auto-generated identifier of the generated document.
- `recipientId` — `string`  
    Autodesk ID of the recipient, who is a project user inside BIM 360 Admin.
- `signedBy` — `string`  
    Not relevant
- `urn` — `string`  
    The version URN of the document with docx format in the Autodesk Forma Data Management service.
- `pdfUrn` — `string`  
    Not relevant
- `status` — `enum:string`  
...
- `errorInfo` — `object`  
  ...
  - `code` — `string`  
      Error code for this document.
  - `message` — `string`  
      Error message to describe this error.
  - `detail` — `string`  
...
---
원본 문서: https://aps.autodesk.com/en/docs/acc/v1/reference/http/cost-documents-GET
