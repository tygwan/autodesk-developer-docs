---
operation_id: cost-budgets-POST
method: POST
...
```http
POST https://developer.api.autodesk.com/cost/v1/containers/:containerId/budgets
```
...
- `code` — `string` **(필수)**  
    Unique code compliant with the budget code template defined by the project admin. Ignored if segmentCodeMap is defined. Max length: 255
- `scope` — `enum:string`  
    The scope of the budget. Possible values are budgetOnly and budgetAndCost.
- `segmentCodeMap` — `object`  
  ...
  - `*` — `string`  
      Max length: 255
- `name` — `string` **(필수)**  
    Name of the budget. Max length: 1024
- `quantity` — `number`  
...
```
curl -v 'https://developer.api.autodesk.com/cost/v1/containers/e94b9bc8-1775-4d76-9b1d-c613e120ccff/budgets' \
  -X 'POST' \
...
---
원본 문서: https://aps.autodesk.com/en/docs/acc/v1/reference/http/cost-budgets-POST
