---
operation_id: cost-performance-tracking-items-id-DELETE
method: DELETE
...
```http
DELETE https://developer.api.autodesk.com/cost/v1/containers/:containerId/performance-tracking-items/:id
```
...
| `id` | array: string: uuid |  | The ID of the budget on which the tracking item to be deleted is based. To find the budget IDs, call GET performance-tracking-items and inspect results.budgetId in the response. |
...
---
원본 문서: https://aps.autodesk.com/en/docs/acc/v1/reference/http/cost-performance-tracking-items-id-DELETE
