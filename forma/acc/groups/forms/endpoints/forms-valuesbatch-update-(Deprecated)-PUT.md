---
operation_id: forms-valuesbatch-update-(Deprecated)-PUT
method: PUT
path: /construction/forms/v1/projects/{projectId}/forms/{formId}/values:batch-update
group: "Forms"
...
```http
PUT https://developer.api.autodesk.com/construction/forms/v1/projects/:projectId/forms/:formId/values:batch-update
```
...
- `tabularValues` — `array: object`  
  ...
  - `id` — `string: UUID` **(필수)**  
      The ID of the table row. You need to generarte the ID (UUID) for the row.
  - `table` — `enum:string` **(필수)**  
      The table to update. Possible values: worklogEntries, materialsEntries, equipmentEntries
...
---
원본 문서: https://aps.autodesk.com/en/docs/acc/v1/reference/http/forms-valuesbatch-update-(Deprecated)-PUT
