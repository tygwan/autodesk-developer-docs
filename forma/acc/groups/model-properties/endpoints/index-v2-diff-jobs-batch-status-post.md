---
operation_id: index-v2-diff-jobs-batch-status-post
method: POST
path: /construction/index/v2/projects/{projectId}/diffs:batch-status
...
- `diffs` — `array: object` **(필수)**  
    diffs. Min items: 1 Max items: 1000
  - `prevVersionUrn` — `string` **(필수)**  
      file version urn.
  - `curVersionUrn` — `string` **(필수)**  
      file version urn.
  - `query` — `object`  
...
---
원본 문서: https://aps.autodesk.com/en/docs/acc/v1/reference/http/index-v2-diff-jobs-batch-status-post
